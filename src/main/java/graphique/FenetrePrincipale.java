...
public class FenetrePrincipale extends JFrame implements ActionListener, WindowListener{
	...
	public FenetrePrincipale(Object sync){
		...
		item_cartes_pioches = new JMenuItem("Cartes pioch�es");
	    ...
	    log_client.add("Application lanc�e :)",Level.FINE);
	    this.initWaitLayer();
	}
...
//						user.refreshListPartie();
						...
						log_client.add("Le client est � null (modifierInterfaceAfterConnexion, FenetrePrincipale)",Level.WARNING);
					}
				}
			});
		    ...
		    bouton_creerPartie.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if(fenetrecreation.isSucceeded()){
						log_client.add("Creation de la partie r�ussie",Level.FINE);
			        	...
			        	log_client.add("Cr�ation de la partie �chou�e",Level.WARNING);
			        } 
	...
	public void afficherToutesLesParties(List<Partie> parties){
    	...
    	for(int i=0; i<parties.size(); i++){ // On cr�er un carr� par parties
...
//						}
						...
						// Ici, on attend que le serveur donne une r�ponse (il fera un setCanJoinParty, puis notify)
//						if(canJoinParty==true){
//							canJoinParty=false; // Pour une r�utilisation
							try{
...
//						}
	...
	public void afficheSalonAttente(String titre_partie, int nbMax, String ... nomsParticipants){
		...
		// Pour donner un effet d'initialisation de la partie, pour que l'affichage ne soit pas instantann� et que
...
//////////// POUR LES TESTS, ON CREE DE FAUSSES CARTES
	...
	/**
	 * Methode permettant d'envoyer une carte � la partie
	 */
	public void soumettreCarte(){  // Elle autorise l'envoie d'une carte � la partie (voir lignes ~ 650)
		boolClickMain=true; 
	...
	/**
	 * Methode permettant d'envoyer une ligne � la partie
	 */
	public void soumettreLigne(){   // Elle autorise l'envoie d'une ligne � la partie (voir lignes ~ 560)
		boolClickLigne=true;
		JOptionPane.showMessageDialog(this,
	...
	/**
	 * Methode prenant en param�tre les cartes que la partie attribue au joueur
	 * et les affiches
	 * @param cartes
	 */
...
///						ENVOYER LE CLICK
	...
	public void afficheCartePiochees(List<Carte> cartes){
		...
		JPanel container_pioche = new JPanel(new BorderLayout()); // Maximum 10 joueurs, donc 5*2 cartes � afficher
	...
	/**
	 * M�thodes permettant de retourner, s'il existe, le composant de nom name (pass� en param�tre) contenu 
	 * dans le container (pass� en param�tre).
	 * @param container
	 * @param name
	 * @return
	 */
	...
	/**
	 * Initialise la fen�tre d'attente de r�ponses du serveur
	 */
	private void initWaitLayer() {  // Le popup d'attente
        ...
        panel.add(new JLabel("En attente de r�ponse du serveur ..."), BorderLayout.CENTER);
	...
	/**
	 * Initialise la fenetre d'informations de l'utilisateur (situ�e dans le panneau � droite)
	 * @return
	 */
	private JPanel initContainerInfo() {
	    ...
	    container_infos.setOpaque(false);
	        ...
	        texte = new JLabel("<html><font color='black'><u>Compte</u> : "+
	        		this.getNomUser()+"<br><br><br>Nombre de parties gagn�es: "+nbgagne+"<br><br>"
	...
	/**
	 * Change le contenu de la fenetre d'info situ� dans la bordure
	 * @param nbTetesBoeuf
	 * @return
	 */
	private JPanel containerInfoDuringPartie(int nbTetesBoeuf) {
	    ...
	    container_infos.setOpaque(false);
	        ...
	        JLabel texte;
	        texte = new JLabel("<html><font color='black'><u>Compte</u> : "+this.getNomUser()+"<br><br>Nombre de t�tes de boeuf <br>actuel : <h1><b><font color='red'>"+nbTetesBoeuf+"</b></font></h1></font></html>");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getActionCommand().equals(item_connexion.getText()) || e.getActionCommand().equals(bouton_connexion.getText())){
			...
			if(fenetreconnexion.isSucceeded()){
				...
				log_client.add("Connect� !",Level.FINE);
				
				// Attente de r�cup�ration des parties du serveur
//				synchronized (sync) {
//					try {
...
//				}
	        	...
	        	is_connected=false;
	        	log_client.add("Connexion �chou�e",Level.SEVERE);
	        }
	    ...
	    // Si c'est une d�connexion
    		...
    		if(choix == JOptionPane.OK_OPTION){
				...
				log_client.add("D�connect� :(",Level.INFO);
			...
			if(fenetreinscription.isSucceeded()){
				...
				log_client.add("Inscription r�ussie",Level.FINE);
	        	...
	        	is_connected=false;
	        	log_client.add("Inscription �chou�e",Level.SEVERE);
	        }
