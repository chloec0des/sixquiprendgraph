...
public class User extends WebSocketClient implements Comparable<User> {
	...
	public void refreshListPartie() {
		...
		send(flag.toString());
		new MonLogClient().add("Mise � jour de la liste des parties ", Level.INFO);
	}
	...
	public void chooseCard(ArrayList<Integer> cards) {
		...
		System.out
				.println("Vous vous �tes tromp� de carte trop de fois! Nous choisissons la carte "
						+ cardValue + " � votre place :D ");
		send(JSONEncode.encodeCarte(userNickname, cardValue, idParty));
		new MonLogClient().add("Une carte a �t� choisie pour vous, qui est la carte :"+cardValue, Level.WARNING);
	}
	...
	public void chooseLine(int idPartie) {
		...
		int row = 1;
		System.out
				.println("Votre carte ne peut �tre plac�e \n Choisissez une rang�e � prendre entre 1 et 4");
		new MonLogClient().add("Impossible de choisir la carte ", Level.INFO);
		new MonLogClient().add("Envoie d'un message au client afin qu'il choisissent une rang�e entre 1 et 4 ", Level.INFO);
		...
		while (true) {
			...
			if (row < 1 || row > 4) {
				...
				new MonLogClient().add("Rang�e non existante", Level.WARNING);
			} else {
				send(JSONEncode.encodeSendRow(userNickname, idPartie, row));
				new MonLogClient().add("Le joueur a choisi la rang�e "+row, Level.WARNING);
				return;
			}
		}
	}
	...
	/**
	 * Envoie au serveur la partie qui vient d'�tre cr�e, par le joueur nomUser
	 * 
	 * @param namePartie
	 * @param nbMaxJoueurs
	 * @param proMode
	...
	public void sendCreationPartie(String namePartie, int nbMaxJoueurs,
		...
		send(JSONEncode.encodeCreatePartie(namePartie, nbMaxJoueurs, proMode,
				nomUser));
		if(proMode){
			new MonLogClient().add("Cr�ation de la partie "+namePartie+" avec "+nbMaxJoueurs+" en mode pro", Level.INFO);
		} else {
			new MonLogClient().add("Cr�ation de la partie "+namePartie+" avec "+nbMaxJoueurs+" en mode pro", Level.INFO);
		}
	}
	...
	/*
	  ...
	  // Ce bloc sert uniquement � avoir un affichage des �l�ments plus "jolie" 
		  try { 
